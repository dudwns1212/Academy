...
public class Test1 {
	...
	public static void main(String[] args) {
		...
		// 변수상자 만들기 (variable)
		// 변수상자를 만드는 이유 : 값(글자나 숫자)가 메모리에 혼자 있을 수 없기 때문에
		// 변수상자가 하는 역할 : 메모리라는 창고같은 공간 안에서 어느 위치에 있는지 정보를 알고 있음
		// C언어 -> 포인터를 이용해서 변수상자의 위치를 알아낼 수 있음
		// 자바언어 -> 포인터가 없고 변수상자의 이름으로만 위치를 알아낼 수 있음
		String name1;
		
		// 변수상자에 값을 넣기
		// name1이라는 변수상자는 12번째 줄에서 이미 만들었고, 여기에서는 변수상자의 이름만으로 상자를 찾아옴
		...
		// 변수상자를 만들면서 값을 넣어주기
		// 처음부터 값을 넣어주기 때문에 초기화라고 함
		int age2 = 22;
		
		// 값을 더해주기
		// + 기호를 사용하면 + 기호 왼쪽과 오른쪽에 있는 값을 더해줌
		// 값이 아니라 변수상자가 오면 변수상자에 들어있던 값이 튀어나와서 더해지게 됨
		int output1 = age1 + age2;
		
		// 콘솔 창에 글자를 뿌려주는 것을 출력이라고 함 (output)
		// + 기호의 왼쪽이나 오른쪽에 글자가 오면 + 기호가 양쪽에 있는 것을 붙여주게 됨
		...
		// 붕어빵 틀에서 붕어빵 만들어내기
		// new 다음에 붕어빵 틀의 이름, 소괄호
		// 붕어빵을 넣어둘 변수상자의 크기는 붕어빵 틀의 이름이 된다
		...
		// 새라는 붕어빵을 만들고 변수상자에 넣을 때 변수상자의 크기를 Bird가 아니라 IAnimal로 만듦
		// IAnimal에서 약속한 함수상자만 사용할 수 있음
		IAnimal bird1 = new Bird();
		
		// walk 함수상자가 IAnimal에서 약속한 것이 아니기 때문에
		// IAnimal 크기의 변수상자에 들어가 있는 새라는 붕어빵에 들어있는 함수라고 하더라도 사용할 수 없음
		// bird1.walk();
		bird1.run();
		
		
		// 약속으로 정한 IAnimal을 이용해서 붕어빵 틀 없이 바로 붕어빵 1마리를 만들 수 있음
		// 약속을 정할 때는 함수상자의 실행코드(중괄호 안에 들어가는 코드)가 없었기 때문에
		// 붕어빵을 만들면서 실행코드를 직접 넣어줌
	...
	static int add(int a, int b) {
		...
		// return을 하면 그 뒤에 있는 값을 함수상자의 아래쪽 구멍으로 떨어뜨려 줌 
		return result;
	}
