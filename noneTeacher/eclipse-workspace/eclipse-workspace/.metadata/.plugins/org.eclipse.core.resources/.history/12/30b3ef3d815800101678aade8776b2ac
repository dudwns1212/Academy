...
public class Computer {
	...
	public static void main(String[] args) { 
		...
		String mix = name + age;
		System.out.println(mix); // name이랑 age을 더함 -> age의 자료형이 문자열로 변환
	    ...
	    System.out.println(computers.get(0).name); // get(index) -> 인덱스번호의 값을 선택하여 이름을 출력
	    
	    // System.out.println(computers.get(0).performance); 오류 발생함-> performance는 Computer 클래스의 객체가 아님, 출력하는 방법은
	    Computer out1 = computers.get(0);  
	    Cpu out2 = (Cpu) out1;  // 다운캐스팅 원래 Cpu타입이던 걸 다시 Cpu타입으로 다운 
	    System.out.println(out2.performance); // 이제 Cpu 타입이므로 배열을 통한 performance 사용가능 
	...
	// 다른 클래스에서 변수 사용하게 하기
	public int age;
	// 위는 그냥 age로 쓰고 아래는 getAge()로 써야됨 
	public int getAge() {
		return age;
	}
