...
public class 메인 {
	...
	public static void main(String[] args) {
		...
		System.out.println(store1.birth + "일 판매정보와 총액");
		
		
		
		// 키가 같아 해시맵이 중복되므로 containsKey(birth)가 키가 없다면 초기화하여 새로 빈 리스트를 만들어 해당 날짜에 등록함
		if (!store1.dayToSoldProductList.containsKey(store1.birth)) {
		    store1.dayToSoldProductList.put(store1.birth, new ArrayList<>());
		}
		 
		// 해시맵리스트애 제품리스트 넣기
		store1.dayToSoldProductList.get(store1.birth).addAll(customer1.myProducts);
		store1.dayToSoldProductList.get(store1.birth).addAll(customer2.myProducts);
		store1.dayToSoldProductList.get(store1.birth).addAll(customer3.myProducts);
		store1.dayToSoldProductList.get(store1.birth).addAll(customer4.myProducts);
		store1.dayToSoldProductList.get(store1.birth).addAll(customer5.myProducts);
		store1.dayToSoldProductList.get(store1.birth).addAll(customer6.myProducts);
		
		
		store1.dayToTotal.put(store1.birth, Customer.totalPrice);
		
		for(int i=0;i<store1.dayToSoldProductList.get(store1.birth).size();i++) {
			System.out.println("판매 제품 : "  + store1.dayToSoldProductList.get(store1.birth).get(i).name + " - 제품 금액 : " + store1.dayToSoldProductList.get(store1.birth).get(i).price);
			
		}
		
		System.out.println("오늘 구매 한 총액 : " + store1.dayToTotal.get(store1.birth));
