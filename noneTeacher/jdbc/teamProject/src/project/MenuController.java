		...
		while(true) { // 숫자 이외의 문자 입력시 오류
			...
			switch (number) {
				...
				case 3 -> {
					...
					int deptId;
					try {
						deptId = Integer.parseInt(num);
						DBMethod.findEmpInfoByDeptId(deptId);
					} catch (NumberFormatException e) {
						System.out.println(new SelectInputException(num).getMessage());
					}
				}
				...
				case 6 -> {
					...
					int selectOrderBy = 0;
					try {
						selectSummary = Integer.parseInt(num);
						DBMethod.getEmpSummaryData(selectSummary, selectOrderBy);	
						if (selectSummary < 0 || selectSummary > 5) {
							throw new SelectNumberException(selectSummary);
						}
					} catch (NumberFormatException e) {
					    System.out.println(new SelectInputException(num).getMessage());
					    continue;					
