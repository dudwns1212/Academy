...
const productSql = require('../database/sql/product-sql');

/**
 * @Controller(path="/product/v1")
 */
module.exports = class Product {
    ...
    async listAll(req, res) {
        logger.debug(`Product::listAll 호출됨.`);

        const sqlName = 'product_list_all';
     ...
     */
    async list(req, res) {
        logger.debug(`Product::list 호출됨.`);

        const sqlObj = productSql.product_list;
     ...
     */
    async read(req, res) {
        logger.debug(`Product::read 호출됨.`);

        const sqlName = 'product_read';
        this.controllerHelper.execute(req, res, sqlName);
     ...
     */
    async add(req, res) {
        logger.debug(`Product::add 호출됨.`);

        const sqlName = 'product_add';
        this.controllerHelper.execute(req, res, sqlName);
    ...
    async modify(req, res) {
        logger.debug(`Product::modify 호출됨.`);

        const sqlName = 'product_modify';
        this.controllerHelper.execute(req, res, sqlName);
    ...
    async remove(req, res) {
        logger.debug(`Product::remove 호출됨.`);

        const sqlName = 'product_remove';
        this.controllerHelper.execute(req, res, sqlName);
    }
